...
dotenv.config();
const USE_CACHED_FILES = process.env.CACHED_DEMO_FILES === "1";

const BIN_DIR = join(import.meta.dirname, "..", "bin");
const FFMPEG = join(BIN_DIR, "ffmpeg", "ffmpeg");
const FFPROBE = join(BIN_DIR, "ffmpeg", "ffprobe");
const FFMPEG_DT = join(BIN_DIR, "ffmpeg-drawtext", "ffmpeg");
...
export async function parseScript(): Promise<Phrase[]> {
...
export async function getAudioDuration(path: string): Promise<number> {
	const { stdout } =
		await $`${FFPROBE} -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ${path}`.quiet();
...
export async function computeScaleExpr(
	...
	const wavBuf = await Bun.file(wavPath).arrayBuffer();
	const wav = Buffer.from(wavBuf);
...
export async function processPhrase(
	...
	if (isPause) {
		...
		await $`${FFMPEG} -y -hide_banner -loglevel error -f lavfi -i anullsrc=r=22050:cl=mono -t ${String(duration)} ${audioPath}`.quiet();
		return {
			agent: "pause",
			audioPath,
			duration,
	...
	const cachedAudioOk =
		USE_CACHED_FILES && (await Bun.file(audioPath).exists());
	const cachedAssOk =
		USE_CACHED_FILES && assPath !== null && (await Bun.file(assPath).exists());
...
async function getBgVideoDuration(): Promise<number> {
	if (_bgVideoDuration !== null) {
		return _bgVideoDuration;
	}
	const { stdout } =
		await $`${FFPROBE} -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 ${BG_VIDEO_PATH}`.quiet();
	_bgVideoDuration = Number.parseFloat(stdout.toString().trim());
	return _bgVideoDuration!;
}
...
export async function renderSegment(
	...
	if (info.scaleExpr !== null && !isPause) {
		...
		const assPart = info.assPath
			...
			: "[base]null[vid]";
		circlePart = `${circleFilter};[bg][circle]overlay=(W-w)/2:${CIRCLE_CENTER_Y}-h/2:format=auto[base];${assPart}`;
	}

	const audioIdx = info.scaleExpr !== null && !isPause ? 2 : 1;
	const mixFilter = `[${audioIdx}:a]volume=2.5[tts];[bga]volume=0.5[bga_v];[tts][bga_v]amix=inputs=2:duration=first[aud]`;

	const parts = [bgFilter];
	if (circlePart) {
		parts.push(circlePart);
	}
	...
	try {
		await $`${FFMPEG} -y -hide_banner -loglevel error ${ffInputs} -filter_complex ${filterGraph} -map [${vidLabel}] -map [aud] -c:v libx264 -preset ${EXPORT_VIDEO_PRESET} -crf ${EXPORT_VIDEO_CRF} -r:v 60 -c:a libmp3lame -b:a ${EXPORT_AUDIO_BITRATE} ${info.videoPath}`.quiet();
	} catch (e) {
		const err = e as { stderr: { toString(): string }; exitCode: number };
		console.error("  FFmpeg error:", err.stderr.toString());
		throw new Error(`Render failed (exit ${err.exitCode})`);
	}
}
...
export async function generateIntroVideo(
	...
	try {
		await $`${FFMPEG_DT} -y -hide_banner -loglevel error -ss 0 -t 3 -i ${bgPath} -f lavfi -i anullsrc=r=22050:cl=mono -filter_complex ${`[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,boxblur=20:5[bg];[bg]${drawtexts},setsar=1[fg]`} -map [fg] -map 1:a -shortest -c:v libx264 -preset fast -crf 23 -c:a aac -b:a 128k ${outputPath}`.quiet();
	} catch (e) {
		const err = e as { stderr: { toString(): string }; exitCode: number };
		console.error("  Intro ffmpeg error:", err.stderr.toString());
		throw new Error(`Intro generation failed (exit ${err.exitCode})`);
	}
}
...
export async function createIntroTransition(
	...
	const introDur = 3;
	const _whooshDur = 0.57;

	try {
		await $`${FFMPEG} -y -hide_banner -loglevel error -i ${introPath} -i ${segPath} -i ${whooshPath} -filter_complex ${`[0:v]trim=0:${introDur - TransitionDur},setpts=PTS-STARTPTS,fps=${FPS},setsar=1[intro_pre];[0:v]trim=${introDur - TransitionDur}:${introDur},setpts=PTS-STARTPTS,fps=${FPS}[intro_tail];[1:v]trim=0:${TransitionDur},setpts=PTS-STARTPTS,fps=${FPS}[seg_head];[1:v]trim=${TransitionDur},setpts=PTS-STARTPTS,fps=${FPS},setsar=1[seg_rest];[intro_tail][seg_head]xfade=transition=slideleft:duration=${TransitionDur}:offset=0[trans];[intro_pre][trans][seg_rest]concat=n=3:v=1:a=0[vid];[0:a]atrim=0:${introDur - TransitionDur},asetpts=PTS-STARTPTS[aud_pre];[2:a]adelay=${(introDur - TransitionDur) * 1000}|${(introDur - TransitionDur) * 1000}[whoosh];[1:a]atrim=${TransitionDur},asetpts=PTS-STARTPTS[aud_post];[aud_pre][whoosh]amix=inputs=2:duration=first[aud_mid];[aud_mid][aud_post]concat=n=2:v=0:a=1[aud]`} -map [vid] -map [aud] -c:v libx264 -preset ${EXPORT_VIDEO_PRESET} -crf ${EXPORT_VIDEO_CRF} -c:a libmp3lame -b:a ${EXPORT_AUDIO_BITRATE} ${outputPath}`.quiet();
	} catch (e) {
		const err = e as { stderr: { toString(): string }; exitCode: number };
		console.error("  Transition ffmpeg error:", err.stderr.toString());
		throw new Error(`Intro transition failed (exit ${err.exitCode})`);
	}
}

export async function applyFisheyeTransition(
	inputPath: string,
	outputPath: string
): Promise<void> {
	const { stdout: probeOut } =
		await $`${FFPROBE} -v error -select_streams v:0 -show_entries stream=r_frame_rate,nb_frames -of csv=p=0 ${inputPath}`.quiet();
	const [fpsStr, nbFramesStr] = probeOut.toString().trim().split(",");
	const fps = Number.parseInt(fpsStr!.split("/")[0]!, 10);
	const _totalFrames =
		Number.parseInt(nbFramesStr!, 10) ||
		Math.ceil(
			Number.parseFloat(
				(
	...
	const splitOut = Array.from(
		{ length: transitionFrames + 1 },
		(_, i) => `[v${i}]`
	).join("");
	...
	try {
		await $`${FFMPEG} -y -hide_banner -loglevel error -i ${inputPath} -i ${WHOOSH_PATH} -filter_complex ${filterGraph} -map [vid] -map [aud] -c:v libx264 -preset ${EXPORT_VIDEO_PRESET} -crf ${EXPORT_VIDEO_CRF} -c:a libmp3lame -b:a ${EXPORT_AUDIO_BITRATE} -r:v 60 ${outputPath}`.quiet();
	} catch (e) {
		const err = e as { stderr: { toString(): string }; exitCode: number };
		console.error("  Fisheye transition error:", err.stderr.toString());
		throw new Error(`Fisheye transition failed (exit ${err.exitCode})`);
	}
}
...
export async function concatSegments(
	...
	const hasMusic =
		bgMusicPath !== undefined && (await Bun.file(bgMusicPath).exists());
	...
	const videoSarLabels = segments
		.map((_, i) => `[${i}:v:0]setsar=1[s${i}]`)
		.join(";");
	const videoLabels = segments.map((_, i) => `[s${i}]`).join("");
	const videoGraph = `${videoSarLabels};${videoLabels}concat=n=${segCount}:v=1:a=0[vid_out]`;

	try {
		await $`${FFMPEG} -y -hide_banner -loglevel error ${videoInputs} -filter_complex ${videoGraph} -map [vid_out] -c:v libx264 -preset ${EXPORT_VIDEO_PRESET} -crf ${EXPORT_VIDEO_CRF} -pix_fmt yuv420p ${videoTemp}`.quiet();
	} catch (e) {
		const err = e as { stderr: { toString(): string }; exitCode: number };
		console.error("Concat video error:", err.stderr.toString());
		throw new Error(`Video concat failed (exit ${err.exitCode})`);
	}
	...
	try {
		await $`${FFMPEG} -y -hide_banner -loglevel error ${audioInputs} -filter_complex ${audioGraph} -map ${audMap} -c:a libmp3lame -b:a ${EXPORT_AUDIO_BITRATE} ${audioTemp}`.quiet();
	} catch (e) {
		const err = e as { stderr: { toString(): string }; exitCode: number };
		console.error("Concat audio error:", err.stderr.toString());
		throw new Error(`Audio concat failed (exit ${err.exitCode})`);
	}

	try {
		await $`${FFMPEG} -y -hide_banner -loglevel error -i ${videoTemp} -i ${audioTemp} -map 0:v:0 -map 1:a:0 -c:v copy -c:a copy -movflags +faststart ${outputPath}`.quiet();
	} catch (e) {
		const err = e as { stderr: { toString(): string }; exitCode: number };
		console.error("Mux error:", err.stderr.toString());
		throw new Error(`Mux failed (exit ${err.exitCode})`);
	}
}
